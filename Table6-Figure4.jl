...
for i in eachindex(r0_all)
    ...
    fig = plot(xlabel="r\n\n($(subfigure_letter[i])) r0 = $(floor(Integer, r0_all[i]))", 
        ...
        leg=false, 
        xlim=(-b,b), 
        ylim=i < 3 ? (-1.5, 1.5) : (-3.5, 3.5),
        xticks=-b:1:b,
        yticks= i < 3 ? (-1.5:0.5:1.5) : (-3:1:3),
        gridalpha=0.5)
    vline!(fig, [-a, a], color=:red, linewidth=4, linestyle=:dot, label="")
...
# Figure 4:
figure4 = plot(subfigures[1], subfigures[2], subfigures[3], layout=(1,3), size=(3000, 1600), plot_title="Second Even Mode with Impedance Boundary Condition\n", plot_titlefontsize=48, bottom_margin=30mm)
display(figure4)
